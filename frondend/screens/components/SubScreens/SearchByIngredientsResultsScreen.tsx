...
export default function SearchResultsScreen({
  ...
  React.useEffect(() => {
    ...
    console.log("Pred Resultssss", route.params.search_keys)
    var route_params = []
    route.params.search_keys.map(item => {
      route_params.push(item.text)
    })
    console.log("route_params ", route_params)
    setIsLoading(true)
    // axios.post('http://10.0.2.2:8000/api/recipe_search_by_ingredient', route.params, {
    axios.post('http://3.131.141.252/api/recipe_search_by_ingredient', {search_keys:route_params}, {
